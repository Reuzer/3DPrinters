...
import CoilsItem from '../coils/CoilsItem.vue';
const coils = defineProps<{ coils: Coil[] }>()
const emit = defineEmits<{
  (e: 'delete', coil: Coil): void,
}>();

function deleteCoil(coil: Coil) {
    emit('delete', coil);
}
</script>
...
<template>
    <div class="content">
        <h2>Plastics</h2>
        <ul v-if="coils.coils.length > 0" class="list">
            <CoilsItem v-for = "coil in coils.coils" :coil="coil" :key="coil.id">
                <div class="btn">
                    <UIButton class="delete__btn" @click="deleteCoil(coil)">
                        Удалить
                    </UIButton>
                </div>
            </CoilsItem>
